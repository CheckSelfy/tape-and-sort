...
//    in_memory_tape intermediate = in_memory_tape(N);
    ...
    std::unique_ptr<tape_base> resulted_tape = factory();
    std::unique_ptr<tape_base> big_tape = factory();
    std::unique_ptr<tape_base> small_tape = factory();

    if (M > 1) {
        copy_and_sort_chunks(from, *resulted_tape, M);
    } else {
        copy_piece(&from, resulted_tape.get(), N);
    ...
    for (std::size_t sorted_piece_size = M; sorted_piece_size < N; sorted_piece_size *= 2) {
        ...
        for (std::size_t cur_chunk = 0;
             cur_chunk < (N + sorted_piece_size - 1) / sorted_piece_size; cur_chunk += 2) {
            small_tape->move_to_start();
            std::size_t moved_big = copy_piece(big_tape.get(), small_tape.get(), sorted_piece_size);
            small_tape->move_to_start();
            if (moved_big == sorted_piece_size) { // if we need merging
                merge(big_tape.get(), small_tape.get(), resulted_tape.get(), sorted_piece_size);
            } else { // we copied only small
                copy_piece(small_tape.get(), resulted_tape.get(), sorted_piece_size);
            }
        }
        resulted_tape->move_to_start();
    ...
    copy_piece(resulted_tape.get(), &to, N);
